...
sudo iptables -t nat -A PREROUTING -i eth0 -p tcp --dport 50022 -j DNAT --to 10.248.0.68:22

# DNAT everything else to FW Untrust except itself
sudo iptables -t nat -A PREROUTING -i eth0 -d 10.248.0.68 -j DNAT --to-destination 10.248.4.4
...
tc/network/if-post-down.d/iptablessave
